...
***
### Overview
To better control HIV infection among high risk population, it urgently needs to explore the relationship between related determinants that may affect Pre-Exposure Prophylaxis (PrEP) usage. Jilinde PrEP cohort study was conducted in Kenya, it contains over 1000 participants and 300 variables that are related to PrEP usage among high risk population. In this project, curated dataset is used to analyze the potential PrEP predictors that can be used as precidor for decision of taking PrEP among targeted population.\
...
### Introduction 
Pre-Exposure Prophylaxis (PrEP) has been proven an effective way to prevent HIV infection.[1] However, the PrEP coverage among high risk population is still low.[2] The reason behind it is complex, because making a behavioral decision such as initiate PrEP is determined by multiple factors such as social determinants, self-perception, and clinical history. To better curb HIV infection among high risk population, it is urgent to identify the key factors that may affect the decision of taking PrEP. Because that will tremendously helps both researcher and local policy makers to advocate PrEP as a main HIV prevention strategy in Kenya.\
\
As aforementioned, because the factors that may influence the decision of taking PrEP are interdisciplinary, when conduct and collect the dataset, the researchers always try to capture as many participant's characteristics as possible. Therefore, it leads a fact that a typical epidemiological dataset usually contains over 100 or sometimes couple hundreds variables. Sometimes, those variables are highly collinearity.As it is proved by previous studies, a precise predictive result is highly depended on model and variable selection, which means that once the outcome is settled, the main challenges for researchers are how to find the significant associated predictors related to outcome and establish the best predictive model to answer the hypothesis they posed.\
\
In recent decade, machine learning method has been widely applied in more and more fields, it also shapes the way how to establish a predictive model in the field of public health.[3] Compared to the traditional way which the variables/predictors selection was heavily relied on the researcher's background knowledge and literature review, and then pick up the appropriated variables from dataset for model they use, using machine learning algorithm not only helps researchers to quickly pick up the most significant associated variables among the hundreds variables, but also enables them to optimize the model based on the rough result, and optimize the model till it is satisfied.[4][5] \
\
In this study, we use this JILINDE cohort study, but only the baseline completed dataset to explore the significant predictor for decision of taking PrEP. By applying the dataset into multiple supervised machine learning models, we expect to compare and evaluate the best performance model for this dataset, and further optimize the selected model. It is expected to through this research, we can provide a references for HIV prevention through PrEP in local area. In addition, we also want to present model optimization method by integrating causal inferential theory to machine learning model. \ 
\

##### Hypothsis
In this research, the **primary aim** is exploring significant predictors for outcome: Decision of taking PrEP (original variable name: agree_prep). And the **secondary aim** is comparing machine learning model's performances given a imbalanced dataset. Specifically, we will compare different machine learning algorithms and the different performance between original selected model and optimized model.\
...
\
**Step 3: Variable selection** Third, we use variables (other than outcome vairiable) as predictors to find statistical significant predictors that are related to outcome at $\alpha$=0.05 level. This process is firstly used GLM model, and then use the random forest tree to replicate the process and collect top significant predictors based on the Gini score. We list and visualize the Gini score result from the RF and P-value from GLM side by side to highlighted how many variables are overlapped in these two methods. \
\
...
**Step 5: Create original GLM model** Fifth, the original model will be created and the model will use the top 6 statistical significant variables/predictors that are generated by GLM model. Meanwhile, these 6 variables must be also showed in Gini score result.\ 
\

**Step 6: Optimize GLM model and evaluation** \
1) Optimize variables in model. We will re-evaluate the model and its independent variables to see which one should be removed. The DAG conception is borrowed in this process. Even though this is a cross-sectional study and we do not specify the exposure. But we treat the all selected variables in original GLM model as a bundle of exposure, then evaluate whether the demographic variables with statistical significant in demographic descriptive table should be also included. The included rule for variable is that if it plays the role like potential confounding in longitudinal study. \
\
2) Display optimized GLM model's summary results. The optimized GLM model and its adjusted predictors will be presented in Table. \
\
3) Compare the model performance. The last step in this section is evaluating and comparing the original GLM model and optimized GLM model.We re-rum the K-cross validation (k=10),  ROC curves as well as AUC to explore whether after optimization, the model's performance is improved.  \
\
The flow chart of method is presented as follow: \
...
#Convert categorical variables as factor and derived age group: agegrp based on continuous var: age
fdat<-df503%>%
...
#Cleaning dataset for map visualization. Create a separate dataset called dfmap. 
dfmap<-df503%>%
...
#### 3-3. Satistic summary of RF and SVM 
```{r}
#Random forest tree 
df.rf <- randomForest(agree_prep ~ ., data =fdat, ntree = 200, importance = TRUE)
df.rf
...
\
Hence, we only use Gini score from RF as reference to see whether the top 6 variables we initially selected are also coverd in Gini result. To bettwer compare results from GLM an RF-Gini, we create a side by side list and plot to present both top 19 variables/predictors which are significant associated with outcome. \

#### 3-4. Create the significant associated variable list based on Random forest and GLM model 
...
#Visualized top 19 significant vars according to their p-value and Gini score

# 1) List top 19 vars based on p-value

plot1<-ggplot(datsig, aes(reorder(Var_Name,-`P-value`, sum), `P-value`)) + 
            geom_bar(stat ="identity")+
            coord_flip()+
            theme_classic()+
            theme(plot.title = element_text(hjust=0, size = 8, face = "bold"))+
            xlab("Top 19 Variable's name: GLM")+
            ggtitle("Selected variables \n that are significant \n  associated  with outcome \n (a=0.05)")
  

# 2) List top 19 vars based on gini score

ginidf<-data.frame(name = names(df.gini), gini = df.gini)%>%
  arrange(gini)%>%
  top_n(19)
plot2<- ggplot(ginidf, aes(reorder(name, gini, sum), gini)) + 
            geom_bar(stat ="identity")+
            coord_flip()+
            theme_classic()+
            theme(plot.title = element_text(hjust=0,size = 8, face = "bold"))+
            xlab("Top 19 Variable's name: RF-gini")+
            ggtitle("Selected variables \n that are significant \n associated with outcome \n (Gini Score)")

# 3) Viz two plots side by side
library(cowplot)
plot_grid(plot1, plot2)
```
\

The next step is visualizing performance for each model we introduced before (GLM, RF and SVM). We use K-cross validation (K=10) to see whether the assumption we made before (Extreme imbalanced dataset will impact the ML performance in a negative way) can be verified based on the results of ROC curve and AUC.Then, we will pick up the best performance model and use the top 6 variables/predictors from the list we created in previous step (see 3-4) to construct our original model. 
\
...
```
\
The result shows that our two assumption is correct. First, the extreme imbalanced dataset indeed lead to very unsatisfied predictive performance for each model (None of them has AUC over 80%). In addition, among the three machine learning models, the SVM has the worst predictive performance, which is consistent with the statistic summary from each model (see 3-3).\

\

##### 4-2. Create original GlM Model based on top 6 significant variables 

Since the GLM has better performance to handle this imbalanced dataset than others, we will use GLM model and the top 6 variables from the GLM calculation which are also appear in RF's Gini result. (See list in 3-4) .\
...
Because we already know the extreme imbalanced dataset can cause unreliable statistic result. We will re-evaluate the all included variables/predictors, the goal in this process is that we want to improve the performance given the current GLM model. It is a relative subjective process since it requires background in this field. To achieve this optimized goal, it needs to do three things: One is re-consider the top 6 variables generated by Gini score (see table 2), to see whether we should also include them into current GLM model; Second, we need to consider and adjust the collinerity issue for current selected variables; The third step is borrowing concept of DAG theory to see whether demographic variables should be added to increase the performance of predictive model.\

1). **Re-consider top variables generated by gini score.** \

According to the Gini score result, we see several variables with prefix of "decline" have significant association with outcome. But we can see that they all have extreme imbalanced outcome number: the number for "yes" is less 10. For example, the top 1 variable: decline_adhere, the number of "yes" is only 9 out of 1132. \

Whereas, the GLM consider more balanced outcome variables, the top 6 variables that we included in model are relative balanced compare to the top 6 variables in Gini score (see table 3), which all small portion of outcome (variable=="yes") are over 25. That is why we should include top 6 variables from GLM instead of RF. Hence, for this first step, we adhere to the GLM model's result and decide NOT to add any of new variables from Gini score result into current model.\
\
...
datgini6<-fdat%>%
  dplyr::select(decline_adhere,decline_other,decline_risk,decline_family,decline_decrease,decline_test)
datgini6%>%
  tbl_summary()%>%
   modify_header(label ~ "**Variable**") %>%
   modify_caption("**Top 6 significant associated vars based on Gini score**")

datglm<-fdat%>%
  dplyr::select(service_received_msm_risk,service_received_msm_hiv,service_received_msm_condoms,risk_low_known,identity_group_msm,risk_low_condoms)
datglm%>%
    tbl_summary()%>%
     modify_header(label ~ "**Variable**") %>%
     modify_caption("**Top 6 significant associated vars based on GLM**")
```
\

2). **Using DAG conception to add demographic variables.** \
According to the demographic (table 1 result), we found that only sex and age group (agegrp) have a statistical significant association with outcome. Even though the directed acyclic graph (DAG) has limited implication for cross-sectional type dataset since it does not refelct temporal realtionship between exposure and outcome,we still want to borrow this conception to see which one or whether both of them should be included into the model if these variables affect both predictors and outcome. \
```{r}
library(dagitty)
library(lavaan)
g <- dagitty('dag {
...
According to the DAG, we can see that agegroup should be included into the model because it affects both current selected vars and outcome given the background knowledge. However, even though the sex shows statistical different in table 1, we do not need to include it because the sex may affect all current selected variables, however, it does not affect the outcome (decision of taking PrEP). In other words, there is no evidence that the women has significant different willingness to take PrEP compared to men or vice versa. \
...
a<-car::vif(glmfdat)
kableExtra::kable (a, caption="Table 5. VIF for variables in original GLM model", digits = 3)%>%
     column_spec(2, bold = T) %>%
     row_spec(1:4, bold = T, color = "black", background = "#d78d1e")%>%
     kableExtra::kable_classic(full_width=F, html_font = "Cambria")
b<-car::vif(glmfdat2)
kableExtra::kable (b,caption="Table 6. VIF for variables in optimized GLM model", digits = 3)%>%
    column_spec(2, bold = T, color = "black", background = "#afd71e")%>%
    kableExtra::kable_classic(full_width=F, html_font = "Cambria")
```
\
In this process, we found the variable "identity_group_msm", "service_recived_msm_hiv", "service_received_msm_risk" have a high collinearity issue: their VIFs are close or higher than 5. So we should adjust them by removing some of these variables to eliminate the collinearity. Based on background knowledge, MSM received service usually contains HIV prevention service, so we remove the variable of "service_received_msm_hiv", similarly reason, we should remove "identity_group_msm".\

After adjusted, we can see the variables VIF decrease to under 2, which is lower than the threshold we use in this research (VIF<2.5).This implies that we successfully fixed collinearity issue.\ 
\
Therefore, our final selected predictive variables are narrow down from 6 to 5, with 1 new added variable age group (agegrp). The table shows the change of variables in original GLM model and optimized GLM model.  \

|**Variables in original GLM**|**Variables in optimized GLM**|**Outcome/dependent variable**| 
|:---|:---|:---|
|service_received_msm_hiv | |agree_prep|
|service_received_msm_risk |service_received_msm_risk|agree_prep |
|risk_low_known|risk_low_known|agree_prep |
|risk_low_condoms|risk_low_condoms|agree_prep |
|risk_low_never|risk_low_never|agree_prep |
|identity_group_msm|agegrp |agree_prep |
...
```{r message=F, warning=F}
  ...
  kableExtra::kbl(cbind(sdf2,z2), caption="Table 7. Summary of optimized GLM model", digits = 3)%>%
    kableExtra::kable_classic(full_width=F, html_font = "Cambria")
```

##### 4-5. Create the summary Table for optimized GLM model
```{r message=F, warning=F}
glmfdat2%>%
  tbl_regression(exponentiate=T, 
                 label=list(service_received_msm_risk="Whether received MSM risk education service",
                            ...
                            risk_low_condoms="I consistently use condoms",
                            risk_low_never~"Never had sex",
                            agegrp="Age group"))%>%
  modify_table_body(~.x %>% 
  ...
  bold_p()%>%
  modify_header(label ~ "**Variable**")%>%
  modify_caption("**Table 8. Statistics of optimized model**") %>%
  italicize_labels()%>%
  bold_labels()
```
\
The result shows that most of variables have statistical significant association to outcome.Those variables can also be used as a good predictors to predict whether the target population are willing to take PrEP or not. Even though we can see strong magnitude for certain variables such as var: Never had sex (OR=18.1), we also notice that 95% CI is extremely wide, some of them also across the 1, which indicates the statistical results is not precise. This is an expected output given the fact that extreme imbalanced dataset aforementioned. \
\

```{r}
#create visulaized coef table for optimized GLM model
coef_table <- data.frame(
  Variables = c("Whether received MSM risk education service (ref=yes)", "I consistently use condoms (ref=yes)", "Never had sex (ref=yes)", "Age group (ref=Adult)"),
  Coefficients = c(12.6, 6.15, 17.5,0.23),
  Conf.Lower = c(3.96, 2.11, 0.85,0.05),
  Conf.Higher = c(36.4, 16.5, 131,0.73)
) 
...
data.frame(
  ...
  kableExtra::kbl(booktabs = T, caption="Figure 2. Coefficient for siginificant predictors from optimized GLM model")%>%
  kableExtra::kable_classic(full_width=F, html_font = "Cambria")%>%
...
The last step for this project is evaluating the performance of optimized model. By comparing to original model, which was only selected the top 6 variables (overlapped with Gini Score result) from GLM result, we use ROC curve and AUC value to see whether the optimized model improves the predictive performance. \
...
```
\
From the plot, we can see that the optimized the model increases the model's performance.The AUC value is increased from 76% to 82.6%. This means that adding variable of age group helps to improve the models performance. \

\

### Results 
\
There are couple of findings based on the this research:\
\
1)**Main result** \
The result shows that in optimized model, 4 variables out of 6 have statistical significant association with outcome. They are:Whether received MSM risk education service(service_received_msm_risk, p-value<0.001, OR=12.6), I consistently use condoms(risk_low_condoms, p-value<0.001, OR=6.15),Never had sex(risk_low_never, p-value=0.014, OR=17.5) and Age group (agegrp, P-value=0.026, OR=0.23).\
...
The result also shows the practical implications. It reveals the key factors regard to PrEP usage among high risk population in Kenya if the local government want to advocate PrEP as a HIV prevention strategy. \
\
2) **Variable selection** \
The Jilinde study has total 4115 obs. and 367 variables. After data cleaning, the final dataset downsized to 1132 observations and 106 variables. Among all of these variables, 19 variables are selected by GLM model as the significant predictors for outcome. And most of the variables are consistent with the result of Gini score result (Table 3-3). \
...
The study compared SVM, Random forest tree and GLM for predictive performance.The result shows that GLM has the best performance for this extreme imbalanced dataset.The worst performance is SVM model, which can not correctly predict for small number of outcome (correctness is zero). \

The GLM and top 6 significant predictors are used as original predictive model, the AUC value and ROC result shows it only has relative good performance (AUC is about 76.0%). After optimization, the optimized GLM model shows that optimization improves the model's performance. Compare to original GLM model, the AUC value is increased to 82.6%. \
\ 

### Discussion

1) **Unbalanced number of outcome** \
Given the fact that this project used the imbalanced data, it proves that machine learning models usually cannot perfectly handle that. Specifically, imbalanced dataset here means that a classification data set with skewed class proportions. According to the previous study, machine learning models will encounter a problem for such type of dataset because when it trains on imbalanced datasets, it often have poor results when they have to generalize (predict a class or classify unseen observations). Moreover, some models will be more susceptible to unbalanced data than others despite of the chosen algorithm.[6] This has been proved in this project as we found in model selection section. To make this result more meaningful in practice, further some advanced methods such as random oversampling should be introduced to fix it.[7] To fix this type of dataset in practice, the advanced method such as random oversampling should be considered.In addition, since the weight in different outcome groups is different, it could be better to evaluate model by using F1 score and AUPRC.  \
\
2) **Completed case**\
This study used a completed dataset to analyze, which may cause biases since the missing values may not lose randomly. In practice, we should consider to adopt the imputation package in R to fix missing value issue if the missing proportion is under the certain percentage, say less than 30%. Moreover, it also shows the importance for researchers about keeping high quality of dataset when they design and collect the data.[8][9] \


### Reference
[1]Donnell, D., Baeten, J. M., Bumpus, N. N., Brantley, J., Bangsberg, D. R., Haberer, J. E., ... & Celum, C. (2014). HIV protective efficacy and correlates of tenofovir blood concentrations in a clinical trial of PrEP for HIV prevention. Journal of acquired immune deficiency syndromes (1999), 66(3), 340.\
[2]Shrestha, R., & Copenhaver, M. (2018). Exploring the use of pre-exposure prophylaxis (PrEP) for HIV prevention among high-risk people who use drugs in treatment. Frontiers in public health, 6, 195.\
...
[4]Singh, A., Thakur, N., & Sharma, A. (2016, March). A review of supervised machine learning algorithms. In 2016 3rd International Conference on Computing for Sustainable Global Development (INDIACom) (pp. 1310-1315). Ieee.\
[5]Ayodele, T. O. (2010). Types of machine learning algorithms. New advances in machine learning, 3, 19-48.\
[6] Cieslak, D. A., & Chawla, N. V. (2008, September). Learning decision trees for unbalanced data. In Joint European Conference on Machine Learning and Knowledge Discovery in Databases (pp. 241-256). Springer, Berlin, Heidelberg. \
[7] Viloria, A., Lezama, O. B. P., & Mercado-Caruzo, N. (2020). Unbalanced data processing using oversampling: Machine learning. Procedia Computer Science, 175, 108-113. \
[8]Gorelick, M. H. (2006). Bias arising from missing data in predictive models. Journal of clinical epidemiology, 59(10), 1115-1123.\
[9]White, I. R., & Carlin, J. B. (2010). Bias and efficiency of multiple imputation compared with complete‐case analysis for missing covariate values. Statistics in medicine, 29(28), 2920-2931.\
